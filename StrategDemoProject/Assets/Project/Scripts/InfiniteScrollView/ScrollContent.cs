...
public class ScrollContent : MonoBehaviour
{
    ...
    private void InitializeContentHorizontal()
    {
        ...
        for (int i = 0; i < rtChildren.Length; i++)
        {
            ...
            childPos.x = originX + posOffset + i * (childWidth + 50);
    ...
    private void InitializeContentVertical()
    {
        ...
        float worldOffset = rtChildren[0].rect.position.y * rtChildren.Length;

        for (int i = 0; i < rtChildren.Length; i++)
        {
            Vector2 childPos = rtChildren[i].localPosition;
            childPos.y = originY + posOffset + i * (childHeight + 50) + worldOffset;
            rtChildren[i].localPosition = childPos;
        }
    }
}
